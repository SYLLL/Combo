...
export default function Index() {
  ...
  const handleFigmaUrlChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (url.trim() && !validateFigmaUrl(url)) {
      setFigmaUrlError('Please enter a valid Figma design URL');
    } else {
      setFigmaUrlError('');
    }
  };
  ...
  const handleUploadSettlements = async () => {
    if (settlementFiles.length === 0) return;
    ...
    try {
      ...
      if (response.ok) {
        ...
        setSettlementUploadStatus({
          type: 'success',
          message: `Successfully uploaded ${result.uploadedCount} settlement files`
        });
        setSettlementFiles([]);
      } else {
      ...
      setSettlementUploadStatus({
        type: 'error',
        message: 'Network error occurred while uploading files'
      });
    } finally {
